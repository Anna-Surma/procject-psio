...
void Enemy2::check_coll(Bullet bullet)
{
...
std::vector<sf::Sprite> Enemy2::create_enemy(std::vector<sf::Sprite> &walls, sf::Texture &enemyTexture)
{
    srand(time(NULL));
...
void Enemy2::draw_enemys(sf::RenderWindow &window)
{
    for(auto &enem : enemys)
    {
        window.draw(enem);
